...
2. **Configure subnets** for the interfaces. For example, if you're using `eth0` with the `192.168.1.0/24` network and `eth1` with the `192.168.2.0/24` network, you would configure the subnets like this:
...
1. **Open the configuration file** for the DHCP server:

   ```bash
   sudo nano /etc/default/isc-dhcp-server
   ```

2. In this file, look for the `INTERFACES` line. By default, this may be empty. Set the interfaces that the DHCP server should listen on. For example:
...
1. **Open the configuration file** for the DHCP daemon:

   ```bash
   sudo nano /etc/sysconfig/dhcpd
   ```

2. In this file, set the `DHCPDARGS` variable to specify which interfaces the server should use. For example:
...
2. **Verify the DHCP server status** to make sure it is running correctly:
...
1. **Check the client’s IP address** after the lease:
